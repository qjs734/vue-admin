...
export default {
  ...
  data() {
    return {
      spu: this.item,
      trademarkList: [], //所有品牌属性
      imageList: [], //所有图片列表展示
      previewImgUrl: "", //图片地址
      visible: false, //图片对话框隐藏
      saleAttrList: [], //所有销售属性列表
      spuSaleAttrList: [], // 当前SPU销售属性列表
      saleAttrValueText: "", //input输入的值
      rules: {
        // 表单校验规则对象
  ...
  computed: {
    ...
    filterSaleAttrList() {
      return this.saleAttrList.filter((sale) => {
        ...
        ); // 适用于数组中是引用类型   用的是字符串与number比较 要转成number
      });
    },
  },
  methods: {
    ...
    saleValidator(rule, value, callback) {
      ...
      // 判断销售属性中至少添加一个销售属性值
      //some方法有一个不行就不行，every全不行才算不行 isNotOk相当于这个table里面的标签为0就返回true 然后退出遍历
    ...
    // addSpuSaleAttr() {
    //   //选中的销售属性id
    //   // console.log(this.spu.saleAttrId);
    //   const { saleAttrId, id } = this.spu;
    //   //再去所有销售属性列表找到某个属性
    //   // const sale = this.saleAttrList.find((sale) => sale.id === saleAttrId.id);
    //   //销售属性添加到商品中
    //     {
    ...
    handleAvatarSuccess(res, file) {
      // this.trademarkForm.logoUrl = res.data;这是更新
      // console.log(file);
      //原数组中添加一张图片
